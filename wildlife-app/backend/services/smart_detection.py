...
try:
    ...
    logger.warning("Image processing libraries not available - some features will be disabled")
...
class SmartDetectionProcessor:
    ...
    def __init__(self, db: Optional[Session] = None):
        ...
        # Species-specific confidence thresholds (lower threshold = more sensitive)
        ...
        self.time_patterns = {
            "deer": {  # Most active at dawn/dusk
                ...
                **{h: 1.2 for h in [5, 6, 7, 18, 19, 20]},  # Dawn/dusk: more active
                **{h: 0.8 for h in list(range(21, 24)) + list(range(0, 5))},  # Night: moderate
            },
            "raccoon": {  # Nocturnal
                **{h: 0.3 for h in range(8, 18)},  # Daytime: less active
                **{h: 1.3 for h in list(range(20, 24)) + list(range(0, 6))},  # Night: very active
            },
            "bird": {  # Diurnal
                **{h: 1.2 for h in range(6, 10)},  # Morning: very active
                **{h: 0.9 for h in range(10, 18)},  # Daytime: active
                **{h: 0.3 for h in list(range(18, 24)) + list(range(0, 6))},  # Night: inactive
            },
        }
        ...
        self.min_contrast = 10  # Minimum contrast (standard deviation of brightness)
    ...
    def check_duplicate_detection(
        ...
        try:
                        ...
                        for det in recent_detections:
                            if det.image_path and os.path.exists(det.image_path):
                                ...
                                if det_img is not None:
                                    det_hash = self._calculate_image_hash(det_img)
                                    similarity = self._compare_image_hashes(current_hash, det_hash)
            ...
            # If same species detected multiple times in short window, might be duplicate
            ...
            return {"is_duplicate": False, "recent_count": len(recent_detections)}
        except Exception as e:
            logger.warning(f"Error checking duplicate detection: {e}")
    ...
    def apply_species_threshold(self, species: str, confidence: float) -> float:
    ...
    def apply_time_pattern_boost(self, species: str, confidence: float, timestamp: datetime) -> float:
    ...
    def process_detection(
        ...
        if image_path:
            ...
            if image_quality.get("is_good_quality", True):
                ...
                analysis["confidence"] = min(1.0, analysis["confidence"] * 1.02)
        ...
        if species != "Unknown":
            analysis["confidence"] = self.apply_species_threshold(species, analysis["confidence"])
        
        # Apply time-of-day pattern if timestamp provided
        if timestamp and species != "Unknown":
            original_confidence = analysis["confidence"]
            analysis["confidence"] = self.apply_time_pattern_boost(species, analysis["confidence"], timestamp)
        ...
        if camera_id and timestamp and image_path and species != "Unknown":
            duplicate_check = self.check_duplicate_detection(image_path, camera_id, timestamp, species)
            ...
            if duplicate_check.get("is_duplicate", False):
                ...
                analysis["duplicate_warning"] = f"Duplicate of detection {duplicate_check.get('duplicate_of')}"
    ...
    def should_save_detection(self, analysis: Dict[str, Any]) -> bool:
        ...
        if image_quality.get("available", False):
            if image_quality.get("quality_score", 1.0) < 0.3:
                logger.debug(f"Not saving detection: very poor image quality ({image_quality.get('quality_score', 0):.3f})")
                return False
        ...
        if confidence >= min_threshold:
            ...
            logger.debug(f"Saving detection: {species} with confidence {confidence:.3f} (threshold: {min_threshold:.3f})")
            return True
        
        logger.debug(f"Not saving detection: confidence {confidence:.3f} below minimum {min_threshold:.3f}")
        return False
